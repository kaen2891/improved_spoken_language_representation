...
parser.add_argument('--epochs', type=int, default=20, help='gpu numbers')
...
## labelled dataset
train_set = TensorDataset(labelled_train, recognized_train, train_label)
...
model_params = list(llm.parameters()) + list(rlm.parameters())
...
model_record_dir = './proposed_models_result/ckpt{}/'.format(args.ckpt)
...
for epoch_i in range(0, args.epochs):
    ...
    for step, (labelled, recognized, labels) in enumerate(train_dataloader):
        optimizer.zero_grad()
    ...
    if args.dataset_name == 'all':
        plot_confusion_matrix(labelled_cmt.numpy(), label_names, title=r'Confusion Matrix of Our Proposed Model$\rightarrow S$ (All of Speakers in {} Dataset)'.format(args.dataset_name2))
        plt.savefig(os.path.join(model_record_dir, 'epoch={},proposed->S,speaker=all'.format(save_epoch)))
    elif args.dataset_name is not None:
        plot_confusion_matrix(labelled_cmt.numpy(), label_names, title=r'Confusion Matrix of Our Proposed Model$\rightarrow S$ ({} in {} Dataset)'.format(args.dataset_name, args.dataset_name2))
        plt.savefig(os.path.join(model_record_dir, 'epoch={},proposed->S,speaker={}'.format(save_epoch, args.dataset_name)))   
    else:
        plot_confusion_matrix(labelled_cmt.numpy(), label_names, title=r'Confusion Matrix of Our Proposed Model$\rightarrow S$ (FSC Dataset)')
    ...
    if args.dataset_name == 'all':
        plot_confusion_matrix(recognized_cmt.numpy(), label_names, title=r'Confusion Matrix of Our Proposed Model$\rightarrow \hat{S}$'+' (All of Speakers in {} Dataset)'.format(args.dataset_name2))
        plt.savefig(os.path.join(model_record_dir, 'epoch={},proposed->S_hat,speaker=all'.format(save_epoch)))
    elif args.dataset_name is not None:
        plot_confusion_matrix(recognized_cmt.numpy(), label_names, title=r'Confusion Matrix of Our Proposed Model$\rightarrow \hat{S}$'+' ({} in {} Dataset)'.format(args.dataset_name, args.dataset_name2))
        plt.savefig(os.path.join(model_record_dir, 'epoch={},proposed->S_hat,speaker={}'.format(save_epoch, args.dataset_name)))   
    else:
        plot_confusion_matrix(recognized_cmt.numpy(), label_names, title=r'Confusion Matrix of Our Proposed Model$\rightarrow \hat{S}$'+' (FSC Dataset)')
        plt.savefig(os.path.join(model_record_dir, 'epoch={},proposed->S_hat,FSC'.format(save_epoch)))
    
    
    with open(os.path.join(model_record_dir, 'proposed_test_result_{}.csv'.format(save_epoch)), 'w') as ff:
